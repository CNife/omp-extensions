 ...
 *   - args -> JSON; collapsed first 2 lines + more, expanded full.
 *   - result -> try JSON.parse: object/array -> JSON, else plain text; collapsed
...
/**
 * TOON 替代：omp 移植版用 JSON 缩进格式渲染结构化值（零外部依赖）。
 * 对象/数组 -> 多行 JSON；其余原样返回字符串。
 */
 ...
 *
 * JSON-parseable object/array -> JSON; everything else (parse fail or
