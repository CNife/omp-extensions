...
export const CONFIG_FILE_NAME = "omp-fff.json";
...
export function loadConfig(agentDir = ompDataDir()): FffConfig {
  ...
  try {
    ...
    throw new Error(
      `Could not read omp-fff config at ${configPath}: ${errorMessage(error)}`,
    );
  }
...
function invalidConfig(configPath: string, reason: string): Error {
  return new Error(`Invalid omp-fff config at ${configPath}: ${reason}`);
}
