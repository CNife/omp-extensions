...
// Resolution order: explicit override > existing fff.nvim db > omp-local data dir.
// Reusing the nvim db lets OMP rank files by the frecency the user built in their editor.
export function resolveDbPaths(overrides: {
  ...
  return {
    frecency:
      ...
      path.join(ompDataDir(), "fff", "frecency"),
    history:
      overrides.history ??
      existingDir(nvimDataDir(), NVIM_HISTORY_DIR) ??
      path.join(ompDataDir(), "fff", "history"),
  };
}
...
export function ompDataDir(): string {
  return process.env.PI_CODING_AGENT_DIR ?? path.join(HOME_DIR, ".omp", "agent");
}
