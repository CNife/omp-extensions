...
- 标签页快捷方法（tab helper）→ 页面对象（page，完整 Puppeteer）→ `code`（全 Node）→ 用户浏览器（relay）。
- 快捷方法不够就换用更底层的能力，别硬凑。
- 用户浏览器（relay）操作属于用户，注意边界。

## 规范

### 拿数据

1. 挂监听：`page.on('response')` 注册处理函数，按网址（URL）特征过滤数据接口。
2. 触发请求：跳转、滚动、切换标签页都会发新请求，监听器持续捕获。
...
- 不要用脚本（JS）提交表单：现代站点拦截，悄悄丢弃，用真实点击。
...
- 监听器没捕获到：先确认请求确实发生，再检查网址（URL）过滤条件。
